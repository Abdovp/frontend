...
export default function DailyChart({ daily }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
      <h3 className="font-heading text-lg font-bold text-ink">الأداء اليومي</h3>
      <div className="mt-4 overflow-x-auto">
        <div className="flex min-w-[640px] items-end gap-2">
          {daily.map((row) => {
            ...
            const label = new Date(row.date).toLocaleDateString('ar-MA', { day: 'numeric', month: 'short' });
            return (
              <div key={row.date} className="flex flex-1 flex-col items-center gap-2">
                <div className="flex h-32 items-end gap-1">
                  <div
                    title={`${row.page_views} زيارة`}
                ...
                <p className="text-[10px] text-slate-400">{row.orders} طلب</p>
              </div>
            );
          })}
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-slate-500">
        <span className="inline-flex items-center gap-2">
          <span className="h-3 w-3 rounded-sm bg-brand/80" /> زيارات
        </span>
        <span className="inline-flex items-center gap-2">
          <span className="h-3 w-3 rounded-sm bg-accent" /> إيرادات
        </span>
      </div>
    </div>
  );

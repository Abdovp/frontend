...
export default function FunnelChart({ steps }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
      <h3 className="font-heading text-lg font-bold text-ink">مسار التحويل</h3>
            ...
            <div key={step.event_name}>
              <div className="mb-1 flex items-center justify-between gap-3 text-sm">
                ...
                <span className="text-slate-500">
                  {step.count.toLocaleString('fr-MA')}
                  {step.rate_from_previous != null ? ` · ${formatPercent(step.rate_from_previous)}` : ''}
                </span>
              </div>
              <div className="h-3 rounded-full bg-slate-100">
                <div
                  className="h-3 rounded-full bg-gradient-to-l from-brand to-brand-light transition-all"
                  style={{ width: `${width}%` }}
                />
              </div>
            </div>
